...
const Quiz = {
  ...
  banks: {
    ...
    supuestos: [
      { q: '¿Qué son los supuestos estadísticos en un modelo de regresión?',
        opts: ['Datos inventados para simplificar el modelo', 'Condiciones que deben cumplirse para que las estimaciones sean válidas y confiables', 'Hipótesis alternativas del investigador', 'Pruebas realizadas antes de recolectar datos'],
        ans: 1, exp: 'Los supuestos son condiciones matemáticas (normalidad, homocedasticidad, independencia, etc.) que garantizan que los estimadores sean eficientes y los intervalos de confianza sean correctos.' },
      { q: '¿Qué afirma el supuesto de normalidad en regresión lineal?',
        opts: ['Que los valores de X deben ser normales', 'Que los residuos del modelo siguen una distribución normal', 'Que la variable Y es siempre simétrica', 'Que la muestra debe ser mayor a 100'],
        ans: 1, exp: 'El supuesto de normalidad se aplica a los residuos εᵢ, no a X ni a Y directamente. Si los residuos son normales, los intervalos de confianza y pruebas t son válidos.' },
      { q: 'Una distribución con asimetría positiva (sesgo > 0) indica que:',
        opts: ['La media es menor que la mediana', 'Los datos tienen una cola larga hacia la derecha y la media > mediana', 'La distribución es perfectamente simétrica', 'Hay más datos a la derecha del centro'],
        ans: 1, exp: 'Sesgo positivo → cola larga a la derecha → media > mediana > moda. Los datos se concentran a la izquierda y se extienden hacia valores altos.' },
      { q: '¿Cómo se llama una distribución más apuntada que la normal (curtosis > 3)?',
        opts: ['Platicúrtica', 'Mesocúrtica', 'Leptocúrtica', 'Asimétrica positiva'],
        ans: 2, exp: 'Leptocúrtica (kurto > 3): distribución muy apuntada con colas pesadas. Platicúrtica (kurto < 3): más aplanada. Mesocúrtica (kurto ≈ 3): igual que la normal.' },
      { q: 'El test de Jarque-Bera evalúa si los residuos son normales usando:',
        opts: ['Solo la media y la varianza de los residuos', 'La asimetría (skewness) y la curtosis de los residuos', 'El rango intercuartílico y la moda', 'El estadístico F del modelo global'],
        ans: 1, exp: 'JB = n/6 · [S² + (K−3)²/4]. Bajo H₀ (normalidad), JB sigue una Chi² con 2 GL. Si p-valor < α se rechaza la normalidad.' },
      { q: 'Si el p-valor del test de Jarque-Bera es 0.02 con α = 0.05, se concluye:',
        opts: ['Los residuos son normales, no hay problema', 'Se rechaza H₀: los residuos NO siguen distribución normal', 'No hay suficiente evidencia para decidir', 'La curtosis es exactamente 3'],
        ans: 1, exp: 'p-valor (0.02) < α (0.05) → se rechaza H₀ de normalidad. Los residuos presentan asimetría o curtosis significativamente diferentes de una distribución normal.' },
      { q: '¿Qué significa el supuesto de homocedasticidad?',
        opts: ['Que los residuos tienen distribución normal', 'Que la varianza de los residuos es constante para todos los valores de X', 'Que no existe correlación entre las variables independientes', 'Que la relación entre X e Y es lineal'],
        ans: 1, exp: 'Homocedasticidad: Var(εᵢ) = σ² constante para todo i. Su violación (heterocedasticidad) produce estimadores ineficientes y errores estándar incorrectos.' },
      { q: 'El estadístico de Durbin-Watson con valor cercano a 2 indica:',
        opts: ['Alta multicolinealidad entre predictores', 'Ausencia de autocorrelación en los residuos', 'Heterocedasticidad grave en el modelo', 'Que el modelo no es significativo'],
        ans: 1, exp: 'DW ≈ 2 → residuos independientes (sin autocorrelación). DW < 1.5 sugiere autocorrelación positiva; DW > 2.5 sugiere autocorrelación negativa. Crítico en datos de series de tiempo.' },
      { q: 'Si el VIF de una variable predictora es 12, esto indica:',
        opts: ['Perfecta linealidad con Y', 'Multicolinealidad grave: esa variable está altamente correlacionada con otras predictoras', 'Baja varianza del predictor', 'El modelo tiene R² = 0.12'],
        ans: 1, exp: 'VIF > 10 es señal de multicolinealidad grave. VIF = 1/(1−R²ⱼ) donde R²ⱼ es el R² de regresar Xⱼ sobre las demás variables. Distorsiona los errores estándar de los coeficientes.' },
      { q: '¿Cómo se verifica el supuesto de linealidad en regresión?',
        opts: ['Calculando el coeficiente alfa de Cronbach', 'Graficando los residuos vs. valores ajustados: deben distribuirse aleatoriamente alrededor de cero', 'Aplicando la prueba de Mann-Whitney', 'Calculando la mediana de los residuos'],
        ans: 1, exp: 'Si la relación es lineal, los residuos graficados contra ŷ no deben mostrar ningún patrón. Un patrón curvo indica falta de linealidad y posible necesidad de transformación o términos polinómicos.' },
      { q: 'Cuando se viola el supuesto de normalidad, ¿qué alternativa es más adecuada?',
        opts: ['Eliminar todas las observaciones extremas sin análisis previo', 'Usar regresión robusta o transformar la variable (log, raíz cuadrada) para estabilizar los residuos', 'Aceptar el modelo sin modificación porque los supuestos son opcionales', 'Aumentar el nivel de significancia α a 0.20'],
        ans: 1, exp: 'Ante violación de normalidad: (1) transformar Y (log, √, Box-Cox), (2) usar métodos robustos, (3) bootstrapping. No se deben eliminar datos sin justificación ni ignorar el problema.' },
      { q: '¿Cuál supuesto es más crítico para la validez de las pruebas de hipótesis en regresión lineal?',
        opts: ['Que n sea mayor que 200', 'La normalidad de los residuos, ya que sustenta las distribuciones t y F usadas para inferencia', 'Que todas las variables sean ordinales', 'Que el intercepto sea distinto de cero'],
        ans: 1, exp: 'Sin normalidad de residuos, los estadísticos t (para coeficientes) y F (para el modelo global) no siguen sus distribuciones teóricas, invalidando los p-valores e intervalos de confianza.' },
    ],
